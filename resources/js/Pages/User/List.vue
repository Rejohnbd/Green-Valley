                ...
                <div class="col-xs-12">
                    ...
                    <div class="box box-primary">
                        <div class="box-header">
                            ...
                            <div class="box-tools">
                                <Link :href="route('users.create')" type="button" class="btn btn-primary btn-flat btn-xs pull-right">Add User</Link>
                            </div>
                        </div>
                            ...
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    <tr v-for="user in users">
                                        ...
                                        <td>
                                            <span :class="getUserRoleClass(user)">{{ user.user_role.name }}</span>
                                        </td>
                                        <td>{{ user.user_info.start_date }}</td>
                                        <td>{{ user.user_info.end_date }}</td>
                                        <td>
                                            <span class="label label-success" v-if="user.active_status">Active</span>
                                            <span class="label label-danger" v-else>Inactive</span>
                                        </td>
                                        <td>
                                            <Link :href="route('users.edit', user)" class="btn btn-social-icon btn-dropbox"><i class="fa fa-edit"></i></Link>
                                            <button class="btn btn-social-icon btn-danger" @click="deleteUser(user)" style="margin-left: 5px" v-if="user.user_role.id != 1"><i class="fa fa-trash"></i></button>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
